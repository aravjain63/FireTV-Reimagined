          ...
          <div className={`${sizeClasses[size]} relative bg-gray-900`}>
            ...
            <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <Button
                size="icon"
                variant="secondary"
                className="w-6 h-6 bg-black/50 hover:bg-black/70 border-0"
              >
              ...
              <div className="mt-2 flex items-center text-xs text-blue-400">
                <div className="flex -space-x-1 mr-2">
                  {showWatchedBy.slice(0, 3).map((friend, index) => (
                    <div
                      key={index}
                      className="w-4 h-4 rounded-xl bg-gradient-to-r from-blue-400 to-purple-500 border border-black flex items-center justify-center text-white text-xs font-bold"
                    >
                      {friend[0]}
                    </div>
                  ))}
              ...
              <div className="flex items-center space-x-2 text-sm">
                <div className="flex -space-x-1">
                  {showWatchedBy.slice(0, 3).map((friend, index) => (
                    <div
                      key={index}
                      className="w-6 h-6 rounded-xl bg-gradient-to-r from-blue-400 to-purple-500 border-2 border-card flex items-center justify-center text-white text-xs font-bold"
                    >
                      {friend[0]}
                    </div>
                  ))}
